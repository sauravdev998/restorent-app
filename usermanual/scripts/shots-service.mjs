...
const sql = (q) =>
  execSync(`docker exec -i manual-db psql -U restaurant_owner -d restaurant -tA`, { input: q }).toString()
...
const admin = await (await browser.newContext({ viewport: { width: 1366, height: 860 }, deviceScaleFactor: 2 })).newPage()
...
await step('choose password', async () => {
  const p = await (await browser.newContext({ viewport: { width: 1366, height: 860 }, deviceScaleFactor: 2 })).newPage()
...
const waiter = await (await browser.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 3, hasTouch: true, isMobile: true })).newPage()
const chef = await (await browser.newContext({ viewport: { width: 1920, height: 1080 }, deviceScaleFactor: 1.5 })).newPage()
...
const openTable = async (n) => {
  ...
  const card = waiter.locator('li', { hasText: new RegExp(`^\\s*${n}\\b`) }).filter({ has: waiter.getByRole('button', { name: 'Open table' }) })
...
const send = async () => {
  ...
  await waiter.getByText(/Round \d/).first().waitFor()
...
await step('table A (late)', async () => {
  ...
  await send()
  sql(`update order_rounds set sent_at = now() - interval '24 minutes', created_at = now() - interval '24 minutes';`)
})
...
await step('table B basket', async () => {
  ...
  await waiter.getByText('Basket, not sent yet').evaluate((e) => e.scrollIntoView({ block: 'start' }))
  await shot(waiter, '22-waiter-basket')
  await send()
  await waiter.getByRole('heading', { name: 'Sent to the kitchen' }).evaluate((e) => e.scrollIntoView({ block: 'start' }))
  await shot(waiter, '23-waiter-sent')
  sql(`update order_rounds set sent_at = now() - interval '12 minutes', created_at = now() - interval '12 minutes' where sequence_no = 1 and sent_at > now() - interval '1 minute';`)
})
...
await step('waiter ready alert', async () => {
  await waiter.getByText(/ready to collect/).first().waitFor()
...
await step('void dialog', async () => {
  ...
  await dlg.waitFor()
  await dlg.getByLabel('Why').selectOption({ label: 'Guest changed their mind' }).catch(() => {})
