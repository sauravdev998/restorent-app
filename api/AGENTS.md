...
| `src/presentation/extract/actor.rs` | Who is asking: the session lookup, the restaurant it scopes to, and the two requirements carried in the handler's own type, the role and the password gate |
...
- **A handler that says nothing about the password is gated.** `Actor<R, P>` carries a `PasswordGate` marker beside the role requirement. `PasswordSettled` is the default and refuses anyone who owes a password change with `403 password_change_required`; only `GET /api/me` and `POST /api/me/password` name `PasswordMayBeOwed`. A new endpoint is covered by writing nothing rather than by somebody remembering. The role is checked first, so a wrong role is still a plain `403`.
- **Every `403` is the role refusal the `Actor` extractor makes before the handler body runs, and nothing else returns one.** When a role may call an endpoint but may not give one value (a chef cancelling a dish may name only `kitchen_unavailable`), the refusal is a `400` naming that field, not a `403` on the role. A refused field value is a named field error everywhere here, and a `403` means only that the caller was the wrong person to ask.
...
- **An editable row carries a `version`, and an edit is a conditional update naming it.** Every write that changes the row bumps the version in the same statement; an edit or rename runs `WHERE id = $1 AND version = $2`, and zero rows means stale or missing, told apart by one follow up read. A stale edit is refused, never merged over somebody else's change.
...
- **Two lock orderings in `db/repository/catalog.rs` are load bearing**, and its module header explains both. Archiving a category locks its row `FOR UPDATE` before counting live dishes while creating, moving, or restoring a dish locks the target category `FOR SHARE`, two modes that conflict, so no live dish can land in a category being archived. Every reorder locks its sibling rows `FOR UPDATE ... ORDER BY id`, a fixed order, so two reorders at once queue instead of deadlocking. Change either and the concurrent transaction tests in `tests/menu.rs` are what catch you.
- **The floor's table lock reaches into service.** Archiving a table in `db/repository/floor.rs` locks it `FOR UPDATE` before looking for an open visit, and `service::lock_live_table` (used by opening and moving a visit) reads the table `FOR SHARE`. The pair is what stops a party being seated at a table removed at the same instant; turn that read back into a plain `SELECT` and nothing fails to compile, only the race tests in `tests/floor.rs` go red. Sections pair with tables the way a menu category pairs with its dishes.
- **One lock order for the service loop: visit, then line or round, then bill, then the bill number counter.** Send takes visit then bill, void and serve take line and round then bill, and close takes the visit `FOR UPDATE` before touching the bill, because closing once took the bill first and could deadlock against a send on the same visit. No path takes a visit lock after a bill lock. The order is written in the module header of `presentation/handlers/service.rs`; anything that locks two bills at once (splitting bills) must fit into it, lower bill id first. The race tests in `tests/waiter_service.rs` are what catch a change here.
- **A failed statement poisons the whole transaction, and nothing here uses a savepoint.** When a range add or a section restore hits the live label index after its own check passed, the handler rolls back, opens a fresh scoped transaction to read the clashing labels, and answers `409 labels_taken` with them in the error body's optional `labels` member. Follow the same shape for any other late unique clash rather than introducing savepoints.
- **Every targeted staff write takes a transaction level advisory lock first**, `lock_restaurant` in `db/repository/staff.rs`, keyed on the restaurant. The two guard rails (nobody acts on their own row, a restaurant is never left without an active admin) are counted and decided inside the writing transaction, so without the lock two admins demoting each other at the same instant could both succeed. The lock is taken for every targeted write, a plain rename included, so all staff writes for one restaurant serialise. That is deliberate and fine at tens of staff. The concurrency tests in `tests/staff.rs` are what catch a change here.
