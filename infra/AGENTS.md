...
- **The load balancer listener is closed to the internet on purpose** (`open: false`), reachable only from the CloudFront managed prefix list. It is one control with the forwarded `CloudFront-Viewer-Address` header, not two: the API throttles sign in attempts by that header, which CloudFront overwrites, so it is trustworthy exactly as long as CloudFront is the only route in. The prefix list id is per region and comes from CDK context (`-c cloudfrontOriginPrefixListId=<id>`); absent, the listener stays open and synth warns loudly rather than failing. Do not ignore that warning on the day this is deployed.
