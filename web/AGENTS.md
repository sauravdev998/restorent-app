...
| `src/app/router.tsx` | The route table, React Router in data mode. Three role guarded groups, plus the three screens outside the shell, and the redirect that sends somebody who owes a password change to `choose-password` before any other screen renders |
...
| `src/waiter/routes/waiter-layout.tsx` | The frame around every waiter screen and the one owner of the ready alert: one always mounted Orders query and one reminder timer, both stopped when the waiter leaves the waiter surface |
...
- **Three screens render outside `RootLayout`**: sign in, register, and the forced password change in `app/routes/choose-password.tsx`. The shell sets `lang` and `dir` and holds the event stream, and none of the three may sit under it: the first two are reached by a signed out visitor, and the third is reached by somebody whose session is refused the stream until they have chosen their own password. `signed-out-shell.tsx` gives them their own frame and they set the document language themselves, which is easy to forget when adding a fourth.
...
- **A value the server will not give back reaches `onSuccess` through the mutation's variables, never through the closure.** TanStack Query replaces a pending mutation's callbacks whenever the owning component re renders, so `onSuccess` runs with the latest render's closure, not the one that called `mutate()`. Reading a form field there can show a value that was never submitted. Pass it to `mutate()` and read it back from the `variables` argument. This is load bearing where the value exists nowhere else: the staff password an admin hands over is never returned by the API and never hashed twice, so the submitted value is the only record of it.
- **Browser storage is a convenience, never a source of truth, and every access sits in try and catch.** The waiter's Mine filter (`localStorage`), and the saved basket with its send key and the acknowledged alerts (`sessionStorage`, per tab), fall back to memory when storage refuses, so a private window or a full disk still works for the page. See `waiter/mine-only.ts`, `waiter/use-saved-basket.ts`, and `waiter/alerts/acknowledged.ts`.
- **One audio context for the whole app, in `shared/ui/audio-unlock.ts`.** A browser allows sound only after a real interaction, so the first tap opens the context and every surface plays through that one: the waiter's ready chime lives in that file and the kitchen's ticket chime takes the context from it (`audioContext()`) rather than opening a second one. Two traps. A screen that shows anything about sound reads `useAudioUnlocked()`, not `isAudioUnlocked()`, because the plain reader is a module flag nothing tells a rendered screen about, and a wall tablet nobody touches again would show "sound is off" for the rest of the shift. And no alert may depend on sound: the visible mark and the announcement carry it on a muted device.
- **Breakpoints follow the viewport, not the column or the type size.** The waiter shell widens from 36rem to 72rem at `lg` (64rem, in `styles/index.css`), so a waiter screen's `lg:` layout is the one that has room; below `lg` it is the phone column whatever the screen. The kitchen's ticket grids take as many columns as fit a 28rem card (`auto-fill`) instead of a column count per breakpoint, because at kitchen type size three viewport sized columns on a laptop squeezed each dish into a sliver. Density (type, targets, spacing) never changes with width.
