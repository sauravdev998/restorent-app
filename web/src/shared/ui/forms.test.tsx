...
describe('Dialog', () => {
  ...
  /**
   * The shape every admin screen uses: the dialog is not in the tree until
   * something opens it, so it mounts already open and never spends a moment
   * closed in which to notice what had focus.
   */
  function MountedOnOpenHarness() {
    const [open, setOpen] = useState(false)
    return (
      <>
        <Button
  ...
  it('hands focus back to what opened it when the dialog mounts already open', async () => {
    const user = userEvent.setup()
    render(<MountedOnOpenHarness />)
  ...
  it('hands focus back when a mounted on open dialog is closed from its own footer button', async () => {
    const user = userEvent.setup()

    // How the admin dialogs actually close: a Cancel in the footer, which
    // unmounts the dialog, rather than Escape.
    function FooterCloseHarness() {
      const [open, setOpen] = useState(false)
      return (
        <>
          <Button
  ...
  it('hands focus back to whichever opener was used last, not the first one', async () => {
    const user = userEvent.setup()

    // One dialog shared by every row, the way a list of people shares a single
    // rename dialog. The second opening must forget the first opener.
    function TwoOpenersHarness() {
      const [open, setOpen] = useState(false)
      const openIt = () => {
        setOpen(true)
      }
      return (
        <>
          <Button onClick={openIt}>Rename Ada</Button>
          <Button onClick={openIt}>Rename Grace</Button>
          <Dialog open={open} onOpenChange={setOpen} title="Rename" />
        </>
      )
    }
    render(<TwoOpenersHarness />)

    const first = screen.getByRole('button', { name: 'Rename Ada' })
    const second = screen.getByRole('button', { name: 'Rename Grace' })
